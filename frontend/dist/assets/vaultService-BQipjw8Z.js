...
import { _ as z, __tla as __tla_0 } from "./pdf-utils-BwD264iA.js";
import { o as F, z as Q, B, C as L, l as D, r as g, b as G, D as w, u as K, s as q, g as W, E as b, a as M, __tla as __tla_1 } from "./index-Dyh55lTL.js";
import { l as x, __tla as __tla_2 } from "./resourceRegistry-DAA6-3RG.js";
import { e as X, r as R } from "./vue-core-Brk0ioAg.js";
    ...
    async function j(t, n = 30, e = 6, r = "SHA-1", s = 0) {
        ...
        try {
            ...
            if (B("using hash-wasm fallback for TOTP generation.")) {
                ...
                let O;
                f.hash === "SHA-256" ? O = _() : f.hash === "SHA-512" ? O = y() : O = m();
                const T = await C(O, o);
                T.init(), T.update(new Uint8Array(u));
                const N = T.digest("hex"), J = new Uint8Array(N.length / 2);
                for(let H = 0; H < N.length; H += 2)J[H / 2] = parseInt(N.substring(H, H + 2), 16);
                d = J.buffer;
            } else {
    ...
    async function Z(t, n = 0, e = 6, r = "SHA-1") {
        ...
        try {
            ...
            if (B("using hash-wasm fallback for HOTP generation.")) {
                ...
                const y = _.digest("hex"), O = new Uint8Array(y.length / 2);
                for(let T = 0; T < y.length; T += 2)O[T / 2] = parseInt(y.substring(T, T + 2), 16);
                u = O.buffer;
            } else {
        ...
        async getTrashList () {
            ...
            try {
                const { useLayoutStore: n } = await z(async ()=>{
                    const { useLayoutStore: o } = await import("./index-Dyh55lTL.js").then(async (m)=>{
                        await m.__tla;
                        return m;
                    }).then((i)=>i.N);
                    return {
    ...
    $ = async (t)=>{
        if (t && t.secret && !t.secret.startsWith("nodeauth:")) {
            const { useAppLockStore: n } = await z(async ()=>{
                const { useAppLockStore: s } = await import("./index-Dyh55lTL.js").then(async (m)=>{
                    await m.__tla;
                    return m;
                }).then((o)=>o.R);
                return {
            ...
            if (r) {
                const { maskSecretFront: s } = await z(async ()=>{
                    const { maskSecretFront: o } = await import("./index-Dyh55lTL.js").then(async (m)=>{
                        await m.__tla;
                        return m;
                    }).then((i)=>i.Q);
                    return {
                        ...
                        if (!p?.success) {
                            ...
                            if (v) {
                                ...
                                const { useOfflineStore: m } = await z(async ()=>{
                                    const { useOfflineStore: y } = await import("./index-Dyh55lTL.js").then(async (m)=>{
                                        await m.__tla;
                                        return m;
                                    }).then((O)=>O.O);
                                    return {
